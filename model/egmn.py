...
class EGMN(torch.nn.Module):
    ...
    def loss(self, y_true, pi, lambda_, mu, sigma):
        ...
        for comp_idx in range(mu.shape[1]):
            ...
            log_prob = normal_dist.log_prob(y_clamped) - torch.log(prob_long.clamp(min=1e-6))
    ...
    def get_quantile(self, pi, lambda_, mu, sigma, tau=0.5):
        mix_pi = torch.softmax(pi, dim=1)

        try_list = torch.arange(0, 1.5, 0.0001).to(pi.device)

        exp_dist = D.Exponential(rate=lambda_.squeeze(1))
        cdf = mix_pi[:, 0:1] * exp_dist.cdf(try_list.unsqueeze(0))

        for k in range(mu.shape[1]):
            norm_dist = D.Normal(mu[:, k:k+1], sigma[:, k:k+1])

            Phi_0 = norm_dist.cdf(torch.zeros(1, 1, device=pi.device))
            Phi_t = norm_dist.cdf(try_list.unsqueeze(0))
