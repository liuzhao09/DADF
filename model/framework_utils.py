...
def discretize_time_label(playtime, split_nodes):
    ...
    binary_labels = _torch.where(cmp_tensor, _torch.ones_like(cmp_tensor), _torch.zeros_like(cmp_tensor))
    return binary_labels.squeeze(1)
...
def restore_time_label(preds, split_nodes):
    ...
    return _torch.sum(preds * bkt_size_list.view([1, -1]), dim=1)
