...
论文实验使用匹配随机种子重复离线比较。公开 runner 会记录每次执行的随机种子和指标，便于新的配对实验保留逐种子日志。
...
本仓库中的便捷脚本通过先 warmup 所选 backbone、再冻结该内存状态来复现 Base 与 DADF 的对比路径。论文中用于 Base、TranSUN 和 DADF 完整对比的多方法实验驱动及其归档逐种子结果不包含在这份参考实现中。表格用于记录论文已报告的实验；新的统计复现应为所有配对方法声明同一组随机种子，并保留生成的日志。
