...
本文中的“纠偏”特指修正可预测的条件偏差，而不是对第一阶段模型进行全局
重校准。视频时长用于索引不同的分布区间，但不被视为预测误差的唯一原因。
...
核心对比实验通过以下控制排除弱 backbone、额外信息和额外参数量对结果的影响：

1. **强 backbone 复现。** 七个第一阶段模型均在接入 DADF 前分别完成调优和验证，
   DADF 并非建立在有意弱化或训练不足的 backbone 上。作为外部 sanity check，
   本仓库复现的 EGMN 在两套公开数据上均优于 EGMN 原论文报告值：KuaiRec
   的 MAE/XAUC 为 `4.081/0.6245`，原论文为 `4.204/0.6093`；WeChat21
   为 `18.330/0.6896`，原论文为 `18.880/0.6692`。由于不同仓库的预处理
   和数据划分可能不同，该对比用于说明复现强度，而不作为严格的方法增益对比；
   完整数值见后文“EGMN 参考结果”。
2. **参数量与信息对齐。** 默认 backbone、扩容 backbone 和 Backbone+DADF
   使用完全相同的预处理样本、训练/验证/测试划分和原始输入特征字段。DADF
   不使用外部数据集或额外的线上推理特征，其辅助目标均由同一批训练样本中
   已有的标签确定性构造。在 10 个随机种子上，将 backbone 的 dense 参数量
   扩展到与 Backbone+DADF 相差不超过 `0.1%` 后，dense 参数量平均增加
   `31.79%`，但平均测试 XAUC 反而下降 `0.0013`，且七个 backbone 中有
   四个没有提升。因此，DADF 的增益不能由额外数据、额外特征或参数量单独解释；
   参数量、复现命令和逐 backbone 结果见后文“Dense 参数量对照”。
3. **严格的二阶段纠偏对比。** 每组 DADF 与 TranSUN 的对比均从完全相同的
   warm-up 后第一阶段参数开始；随后冻结全部第一阶段参数，仅更新各自新增的
   二阶段纠偏参数，公开 DADF runner 默认执行该冻结策略。就 backbone 初始化
   而言，直接复用同一份内存中的 warm-up 后参数，与加载同一个序列化
   backbone checkpoint 等价；是否写入磁盘不会改变该对比的公平性。
...
- 进入纠偏阶段后冻结第一阶段预测器；
- 根据验证集 XAUC 选择最佳纠偏模型；
- 在每组成对比较中，DADF 与适配后的 TranSUN 基线均从同一份 warm-up 后的
  第一阶段参数开始训练，且这些参数在纠偏训练期间保持冻结；
- 配对实验使用相同的数据划分和随机种子，warm-up 后仅更新各自新增的纠偏模块参数。

独立的 `MODE=base` 命令用于纯 backbone 复现与参数量对照，不属于上述纠偏
方法的成对比较流程。
...
和验证集选择协议；再将匹配版本与 Backbone+DADF 的参数量进行对照，从而
区分模型容量效应与 DADF 结构设计。
...
最优的模型参数后进行测试。`Delta XAUC` 定义为“参数量匹配版减去默认
容量版”，因此正值表示扩充参数后更好。
...
backbone 并不能复现 DADF 的增益。这些结果表明，DADF 的增益不能仅由额外
数据、额外特征或参数量解释，而与其如何对这些参数进行分布感知组织与训练
有关。
